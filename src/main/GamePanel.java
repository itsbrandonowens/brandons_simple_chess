...
public class GamePanel extends JPanel implements Runnable {
    ...
    private void simulate() {
        ...
        // This is for restoring the removed piece during simulation. When the player
        // hovers over the piece but isnt completing his move
        copyPieces(pieces, simPieces);
    ...
    private boolean isKingInCheck(){
        Piece king = getKing(true); // opponents king
    ...
    private boolean isCheckmate(){
        ...
        if(kingCanMove(king)){
            ...
            // Check if the player can block the path with another piece
            ...
            if(colDiff == 0){
                ...
                if (checkingP.col<king.col) {
                    // the checking piece is the left 
                ...
                if (checkingP.col>king.col) {
                    // the checking piece is the right
                    for(int col = checkingP.col; col >king.col; col--){
                        for(Piece piece : simPieces){
                            if(piece != king && piece.color != currentColor && piece.canMove(col, checkingP.row)){
                                return false; 
                ...
                if(checkingP.row < king.row){
                    //Checking piece is diagonally above the king 
                    if(checkingP.col < king.col){
                        //upper left
                ...
                if(checkingP.row > king.row){
                    //Checking piece is diagonally below the king 
                    if(checkingP.col < king.col){
                        //lower left
                        for(int col = checkingP.col, row = checkingP.row; col< king.col; col++, row--){
                            for(Piece piece : simPieces){
            ...
            } // Knights attacks cannot be blocked with other pieces therefore it is not needed
        }
        return true;
    }
    ...
    private boolean isValidMove(Piece king, int colPlus, int rowPlus){
        ...
        //Reset the kings positon and restore the removed piece
        king.resetPosition();
        copyPieces(pieces, simPieces);
        return isValidMove;
    }
    ...
    private void changePlayer() {
        if (currentColor == WHITE) {
            currentColor = BLACK;
            // Reset blacks twoStepped status
            ...
            currentColor = WHITE;
            // Reset whites twoStepped status
            for (Piece piece : pieces) {
    ...
    private void promoting() {
        if (mouse.pressed) {
            for (Piece piece : promoPieces) {
                if (piece.col == mouse.x / Board.SQUARE_SIZE && piece.row == mouse.y / Board.SQUARE_SIZE) { // this
                                                                                                            // means the
                                                                                                            // mouse
                                                                                                            // must be
                                                                                                            // on one of
                    ...
                    simPieces.remove(activeP.getIndex()); // Remove the pawn so it can be replaced
                    copyPieces(simPieces, pieces);
                    activeP = null;
                    promotion = false;
                    changePlayer();
